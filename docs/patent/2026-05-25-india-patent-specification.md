...
*(Specialisation: the invention's novel core is the forensic, offline-verifiable warning certificate and its non-repudiable chain. Its hyper-specialised embodiment (Claim 0A) is a verifiable evacuation-ROUTE clearance: a per-segment proceed / caution / do-not-proceed determination for the path from a person to their nearest shelter, issued as an offline-verifiable, tamper-evident certificate. The multi-hazard fusion and forecasting are the inputs the certificate makes trustworthy and accountable.)*
...
- **(M7) Self-improving outcome-verified recalibration ("World Engine"), as a competing ENSEMBLE:** the system records each probabilistic forecast and later confirms whether the predicted event occurred using MULTIPLE independent mechanisms (fused consensus magnitude + cross-sensor corroboration count + divergence flag), scoring both binary skill (Brier, hit/false-alarm) AND continuous closeness (how near the forecast was to the realized magnitude). A plurality of recalibration engines with different adaptation strategies train SIMULTANEOUSLY on each confirmed outcome; the engine with the best verified skill leads and supplies the live calibrated probability. Learned parameters and skill aggregates persist, so accuracy improves across restarts. Technical effect: a measurably improving, self-correcting, competitively-selected forecast.
- **(M6) Forensic warning certificate + non-repudiable chain (the specialisation):** each issued warning is packaged as a self-contained certificate embedding its own verification public key, and each warning's signature is chained to the prior warning's hash. The chain makes the *sequence* of warnings tamper-evident (no reorder/insert/delete/backdate), the embedded key makes each certificate verifiable **offline by any third party** with no server and no distributed ledger, and the location and timestamp are bound by the signature. This yields a non-repudiable, court/insurer-checkable record of exactly what was warned, where, and when, which is the differentiator that does not exist in surveyed systems.
...
**Independent Claim 0A (hyper-specialised: verifiable evacuation-route clearance).**
A computer-implemented method of certifying the safety of an evacuation path, comprising:
(a) receiving an origin location and determining a destination, the destination being a relief facility selected by proximity to the origin;
(b) sampling a sequence of waypoints along a path between the origin and the destination;
(c) obtaining traversal-hazard data for the waypoints by (i) a single bounding-box query returning active-fire detections from a satellite thermal source covering all waypoints, and (ii) batched multi-point queries returning precipitation, wind and surface-elevation values for the waypoints;
(d) for each active-fire detection, determining its observation age and projecting its hazard front FORWARD by a physical spread rate parameterised by satellite-derived dryness, wind and terrain slope, over a horizon comprising the detection's elapsed age plus a time to traverse the route, such that a more latent detection yields a larger projected danger zone; and classifying each path segment between consecutive waypoints into a clearance level among at least proceed, caution, and do-not-proceed, as a function of the distance from the segment to the projected fire front, precipitation intensity weighted by relative low-lying elevation of the segment, and wind speed, and deriving an overall route verdict as the worst segment level;
(d1) computing a confidence that decreases with the observation age of the freshest contributing detection, and, when the satellite layer is absent or older than a staleness threshold, biasing the verdict conservatively by precluding a confident "proceed" determination; whereby observation latency widens the danger margin and never produces a false clearance;
(e) binding the origin, destination, per-segment levels and verdict into a deterministic canonical encoding, signing it with an asymmetric private key, and chaining the signature to a preceding issued clearance to form a tamper-evident, non-repudiable record; and
(f) emitting a self-contained route-clearance certificate, embedding the verification public key, that a field party verifies OFFLINE with no server and no distributed ledger,
whereby a person or responder obtains, and can later prove, a tamper-evident determination of whether a specific evacuation route was safe to traverse at a specific time.

> **Why hyper-specialised and novel:** surveyed hazard systems answer "what is the hazard at a point"; none issues a per-segment, traversal-time evacuation-ROUTE clearance verdict as an offline-verifiable, non-repudiable certificate snapping to the nearest shelter. The specific combination of bounding-box fire + batched multi-point weather/elevation + low-lying-weighted segment classification + the forensic certificate is the narrow inventive core.
...
20. Wherein the certificate of Claim 0A is a route-clearance certificate whose bound content comprises the ordered path segments and their clearance levels, such that altering any segment's safety determination invalidates the certificate.
21. Wherein the destination of Claim 0A is selected as the relief facility nearest the origin from a maintained facility set, and the certificate records the destination identity.
22. Wherein the projected fire front of Claim 0A(d) has a radius increasing with the product of the physical spread rate and the sum of the detection's observation age and the route traversal time, so that a stale detection enlarges, rather than removes, the danger zone.
23. Wherein the certificate records the observation age and the computed confidence as part of the bound or accompanying record, so that the temporal basis of a clearance is itself accountable and auditable.

**Independent Claim 0B (self-improving outcome-verified forecasting).**
A computer-implemented method of improving hazard-forecast accuracy by verification against realized outcomes, comprising:
(a) for a location-cell and hazard, recording a probabilistic forecast produced from fused Earth-observation signals;
(b) subsequently determining, from a later observation of the same location-cell and hazard, a binary outcome of whether the forecast event materialized, by comparing a later observed hazard magnitude against an event threshold;
(c) scoring forecast skill from the recorded probability and the binary outcome using at least a Brier score and a hit/false-alarm rate;
(d) updating, by an online logistic recalibration, parameters of a mapping from raw forecast probability to a calibrated probability so as to reduce the discrepancy between predicted probabilities and observed event frequencies; and
(e) applying the updated mapping to subsequent forecasts and persisting the parameters and skill aggregates so the learning is retained across stateless restarts,
whereby the system measurably improves its calibrated forecast accuracy over time from whether predicted events actually occurred.
24. Wherein step (d) reduces a Brier score of the calibrated probabilities below that of the uncalibrated probabilities, the reduction being reported as a skill gain.
25. Wherein the skill scores and the learned calibration are exposed via an interface that discloses, per hazard, the number of verified events and whether calibration is improving accuracy, and indicates an unlearned state below a minimum verified-event count.
26. Wherein the forecast probability carried by a warning certificate of Claim 0 is the calibrated probability of step (d), binding the system's self-verified skill into the issued, verifiable record.
27. Wherein step (d) is performed by a plurality of recalibration engines trained simultaneously on the same observed outcomes, each engine having a distinct adaptation strategy, and the engine with the best verified skill is selected to supply the calibrated probability for subsequent forecasts; whereby competing self-learning models race on live outcomes and the best-performing model leads.
28. Wherein the binary outcome of step (b) is confirmed by a plurality of independent mechanisms comprising a fused multi-sensor consensus magnitude and a count of independent corroborating sensors, a forecast event being counted as confirmed only when corroborated by at least two independent sensors, and an outcome's influence on learning being weighted by its confirmation confidence.
29. Wherein the scoring of step (c) additionally computes a continuous closeness between the forecast probability and the realized magnitude, so the engines learn from HOW CLOSE each forecast was, not only whether the event occurred.
30. Wherein one of the confirmation mechanisms of step (b)/Claim 28 is an authoritative official-alert source comprising (i) a worldwide official disaster-alert feed matched to the location by geographic coordinates, (ii) for a location within a jurisdiction having a coordinate-queryable official alert service, that service's alerts matched by coordinates, and (iii) for a location within a jurisdiction having a national alerting aggregator without coordinates, that aggregator's feed carrying the jurisdiction's official meteorological, hydrological and disaster-management channels matched to the location by its reverse-geocoded administrative-area name; an official alert for the hazard near the location confirming the event at full confidence independent of the system's own sensors, and the confirming channel being recorded.
31. Wherein the recalibration engines of Claim 27 are maintained independently per geographic region, the region determined by partitioning coordinates into climate-zone buckets, so that the learned probability calibration adapts to regional differences in how a given fused signal corresponds to a realized event; whereby forecast accuracy improves locally for each region of the world rather than by a single global calibration.
