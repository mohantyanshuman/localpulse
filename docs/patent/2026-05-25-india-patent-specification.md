...
- **(M7) Self-improving outcome-verified recalibration ("World Engine"):** the system records each probabilistic forecast, later verifies whether the predicted event actually occurred (observed magnitude vs an event threshold), scores its own skill (Brier, hit/false-alarm), and recalibrates the probability mapping online so predicted probabilities track observed frequencies; the learned parameters and skill are persisted so accuracy improves across restarts. Technical effect: a measurably improving, self-correcting forecast.
