...
ABSTRACT = [
    ...
    (
        "The system is a real-world-aware decision support system. It ingests live "
        "local news through the Google News feed, then uses a Gemini Flash-Lite "
        "language model to filter noise, classify each item by category (road, "
        "shelter, power, water, medical, rumour) and severity, geolocate the place "
        "named, and translate the result into five languages (English, Hindi, "
        "Punjabi, Tamil, Bengali). It fuses this with real hazard feeds: live "
        "weather and derived warnings from Open-Meteo, recent earthquakes from the "
        "United States Geological Survey, and official Indian government alerts from "
        "the National Disaster Management Authority Sachet feed, filtered to the "
        "region. A rule-based decision engine combines everything into a single town "
        "risk level and a ranked list of actionable recommendations. Real relief "
        "points (hospitals, police stations, community centres, schools) are pulled "
        "from OpenStreetMap. Residents submit reports that persist to Firestore and "
        "appear on the shared map within seconds. A multilingual voice assistant "
        "answers questions grounded in this live data."
    ),
    (
